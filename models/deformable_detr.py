...
class SetCriterion_semi(nn.Module):
    ...
    def forward(self, outputs, targets, indicators=None):
        ...
        print("num_boxes:", num_boxes)
        print("== checkpoint 5 ==")
        print(f"Process {torch.distributed.get_rank()} reached all_reduce.")
        ...
        #     future = executor.submit(diagnostic, num_boxes)
        #     try:
        #         result = future.result(timeout=10)
        #         print("= num_box check passed =")
        #     except concurrent.futures.TimeoutError:
        #         print('Function took too long to complete.')
        #         return None
        print("num_boxes:", num_boxes)
        print("get_world_size():", get_world_size())
        temp = num_boxes/get_world_size()
        print("num_boxes/get_world_size():", temp)
        temp2 = torch.clamp(temp, min=1)
        print("torch.clamp():", temp2)
        num_boxes = temp2.item()
...
def diagnostic(problem):
    print("Diagnostic variable object type:", type(problem))
    # print("Diagnostic variable properties:\n", dir(problem))
    print("Diagnostic variable shape:", problem.shape)
    print("Diagnostic variable value:", problem)
    return True

class TimeoutThread(Thread):
    def __init__(self, target, args=(), timeout=5):
